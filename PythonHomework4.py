...
list_2 = []
for i in range(n):
    a = int(input('Введите элементы списка n: '))
    list_1.append(a)
for i in range(m):
    b = int(input('Введите элементы списка m: '))
    list_2.append(b)
print(list_1)
print(list_2)
...
m = list(map(int, input('Введите количество ягод на кустах: ').split()))
a = 0
i = 1
for i in range (1, n - 1):
    if (m[i] + m[i + 1] + m[i - 1] > a):
        a = m[i] + m[i + 1] + m[i - 1]
print(f'Ответ: {a}')
